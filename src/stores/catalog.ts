...
export const useCatalogStore = defineStore('catalog', () => {
  ...
  /**
   * Snapshot the current machine record and all sub-records for rollback purposes.
   */
  async function _snapshotMachine(id: string): Promise<{
    success: boolean
    data?: {
    ...
    if (machErr) {
      return { success: false, error: `Failed to snapshot machine: ${machErr.message}` }
    }

    const { data: features } = await supabase.from('machine_features').select('*').eq('machine_id', id)
    const { data: consumables } = await supabase.from('machine_consumables').select('*').eq('machine_id', id)
    const { data: inclusions } = await supabase.from('machine_inclusions').select('*').eq('machine_id', id)
    const { data: exclusions } = await supabase.from('machine_exclusions').select('*').eq('machine_id', id)
    const { data: addons } = await supabase.from('machine_addons').select('*').eq('machine_id', id)
  ...
  /**
   * Rollback a machine to its snapshot state. Restores main record fields
   * and re-inserts previous sub-records.
   */
  async function _rollbackMachine(
    id: string,
    snapshot: {
      machine: Record<string, unknown>
      features: Record<string, unknown>[]
    ...
    // Revert main record fields that were changed
    if (Object.keys(changedFields).length > 0) {
      const revertFields: Record<string, unknown> = {}
      for (const key of Object.keys(changedFields)) {
        if (key !== 'updated_at') {
          revertFields[key] = snapshot.machine[key]
        }
      }
      revertFields.updated_at = snapshot.machine.updated_at
      await supabase.from('machines').update(revertFields as never).eq('id', id)
    }

    // Restore sub-records from snapshot
    const subTables = [
      { key: 'features' as const, table: 'machine_features' as const },
      { key: 'consumables' as const, table: 'machine_consumables' as const },
      { key: 'inclusions' as const, table: 'machine_inclusions' as const },
    ...
    for (const { key, table } of subTables) {
      // Delete whatever partial state exists
      await supabase.from(table).delete().eq('machine_id', id)

      // Re-insert from snapshot
      const records = snapshot[key]
      if (records.length > 0) {
        await supabase.from(table).insert(records as never)
      }
    }
  }
